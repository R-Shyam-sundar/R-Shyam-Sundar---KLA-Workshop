...
vector<int> get_sides(vector<pair<int,int> >& v)
{
    vector<int> ans;
    for(int i=1;i<v.size();i++){
        int dist = distance(v[i].first,v[i].second,v[i-1].first,v[i-1].second);
        ans.push_back(dist);
    }
    return ans;
}

int get_hash(vector<int>& s) {
    int n = s.size();
...
void milestone2()
{
    ...
    for(int i=0;i<vp.size();i++){
        vector<int> val = get_sides(vp[i].vertices);
        debug(val);
        int h = get_hash(val);
        hs.insert(h);
        auto itr = mp.find(h);
        if(itr == mp.end()){
            mp.insert({h,vector<int>()});
            mp[h].push_back(i);
        }else{
            mp[h].push_back(i);
        }
    ...
    for(int i=0;i<POI_vp.size();i++){
        vector<int> val = get_sides(POI_vp[i].vertices);
        debug(val);
        int h = get_hash(val);
        hs1.insert(h);
        auto itr = mp.find(h);
        if(itr == mp.end()){
            mp1.insert({h,vector<int>()});
            mp1[h].push_back(i);
        }else{
            mp1[h].push_back(i);
        }
